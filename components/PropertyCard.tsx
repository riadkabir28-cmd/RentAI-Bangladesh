...
import { ShieldCheck, UserCheck, MapPin, Bed, Bath, Sparkles, Send, CheckCircle2, Clock, MessageSquare } from 'lucide-react';
...
export const PropertyCard: React.FC<PropertyCardProps> = ({ property }) => {
  ...
  const handleMessage = () => {
    // Navigate to dashboard messages tab (simulated by path)
    navigate('/dashboard');
    // In a real app, we would pass state to open the specific conversation
  };
    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all group flex flex-col h-full">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-4 mb-4 border-y border-gray-50 py-3">
          ...
          <div className="ml-auto text-xs font-medium text-gray-400 px-2 py-0.5 bg-gray-100 rounded">
            {property.type}
          </div>
        </div>
        ...
        {property.aiReason && (
          <div className="mb-4 bg-emerald-50/50 p-3 rounded-lg border border-emerald-100/50">
            <p className="text-xs text-emerald-800 italic leading-relaxed line-clamp-2">
              "AI: {property.aiReason}"
            </p>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3 mt-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="bg-gray-100 text-gray-600 py-2.5 rounded-xl font-semibold hover:bg-gray-200 transition-colors text-sm">
              Details
            </button>
            <button 
